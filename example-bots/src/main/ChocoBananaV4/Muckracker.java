...
public class Muckracker extends UnitMethods {
            ...
            if (rc.canSenseLocation(neighbour)) {
                ...
                if (robot != null && robot.type == RobotType.ENLIGHTENMENT_CENTER) {
                    ...
                    if ((parentFlag & Flags.FOUND) == Flags.FOUND){
                        ...
                        dir = dir.opposite();
                        scoutTarget = current.translate(dir.getDeltaX()*40, dir.getDeltaY()*40);
                        switch (dir) {
                            case WEST:
                                scoutDirectionFlag = Flags.LEFT;
                                break;
        ...
        for (RobotInfo robot : rc.senseNearbyRobots(r, rc.getTeam())) {
            ...
            if (count< 14) {
                if (rc.canGetFlag(robot.ID)) {
                    ...
                    if ((allyFlag & Flags.NOT_SCOUTABLE) == Flags.NOT_SCOUTABLE && ((allyFlag & scoutDirectionFlag) == scoutDirectionFlag)){
                        done = true;
                    }
    ...
    public void readParent() throws GameActionException{
        ...
        if (rc.canGetFlag(parentID)){
            ...
            } else if ((obtainedFlag & Flags.FOUND) == Flags.FOUND && done){
                done = false;
                found = false;
    ...
    public void run() throws GameActionException{
        ...
        if ( rc.getRoundNum() > 100 && allyNear < 3 && ((flag & Flags.NEARSELFCENTER)==Flags.NEARSELFCENTER)){
            vector.update(origin, current, 1000);
        } else if (!done) {
            if (found) {
                flag |= Flags.FOUND;
        ...
        if (nearEdge){
            ...
            Direction newDir = directions[(int) (Math.random() * 8)];
            scoutTarget = current.translate(newDir.getDeltaX()*40, newDir.getDeltaY()*40);
            switch (newDir) {
                case WEST:
                    scoutDirectionFlag = Flags.LEFT;
                    break;
