...
public class EC extends UnitMethods {
    ...
    static final int BID_START = 800;
    int lastbid=0, lastvote=-1, dvote=0;
    static double       SAFE_SLANDERER_PROB = 1.0,
                        EXPLO_PROB = 0.7,
                        TRADEOFF = 0.2,
                        ROBOT_WEIGHT = 0.3, // TRADEOFF+ROBOT_WEIGHT < 1!!!!
                        ROBOT_MAX = 0.5,
                        POLITICIAN_RUSH_PROB = 0.8;
    int foundMemoryCount = 0, FOUND_MEMORY_LENGTH=150;
    ...
    public void buildRobot() throws  GameActionException {
        ...
        if (robot != null) {
            for (int i = 0; i < 8; i++) {
                ...
                } else if (robot == RobotType.SLANDERER) {
                    dir = okDirection();
                } else {
                    dir = nextDirection();
                }
    ...
    public RobotType chooseRobot() throws GameActionException {
        ...
        double explo_prob = EXPLO_PROB;
        if (rc.getRoundNum() > 1500 || rc.getRobotCount()> 250){
            explo_prob *= 0.5;
        }


        if (((spawned%50 == 0) || (rc.getRoundNum() < 100*rc.sensePassability(rc.getLocation()) && foundMemoryCount==0))
                && !NEAROPP && inf >=  (int) Math.max(130, inf * 0.8)){
            robot = RobotType.SLANDERER;
            spawnInfluence = (int) Math.max(130, inf * 0.8) ;
        } else if ((Math.random() < explo_prob) && (foundMemoryCount==0 || Math.random() < 0.2 )){
            ...
            spawnInfluence = (int) (inf * 0.1);
            spawnInfluence += 10;
            if ( (polSpawned % 2)  == 0 && inf > (int) (0.7 * inf) + 50) {
                spawnInfluence = (int) (0.7 * inf) + 50 ;
            }
        ...
        } else if ( (rc.getRoundNum() > 50 && rc.getRoundNum() + 25 < 3000 && (((flag & Flags.NEARMUCK) != Flags.NEARMUCK) ||
        ...
        if (inf < spawnInfluence+MIN_REQUIRED_INFLUENCE && robot !=RobotType.SLANDERER ){
            return null;
        }
    ...
    public Direction nextDirection() throws GameActionException {
        Direction dir = directions[ (direction++) % 8];
        MapLocation current = rc.getLocation();
        int count = 0;
        while (count < 8 && rc.isLocationOccupied(current.add(dir))){
            dir = directions[ (direction++) % 8];
            count ++;
        }
        return dir;
    }
    ...
    public Direction okDirection() throws GameActionException {
        int index, count = 0, scoutableSize = scoutable.size();
        Direction dir;
        MapLocation current = rc.getLocation();
        index = scoutableDirection - 1;
        dir = scoutable.get( ((index) % scoutableSize + scoutableSize)%scoutableSize);
        while (count < scoutableSize && rc.isLocationOccupied(current.add(dir))) {
            index--;
            dir = scoutable.get(((index) % scoutableSize + scoutableSize)%scoutableSize);
            count++;
        }
        ...
        for(int ID: tracked){
            if (rc.canGetFlag(ID)) {
                ...
                if ((obtainedflag & Flags.FOUND) == Flags.FOUND) {
                    ...
                    if (foundMemoryCount== 0 || d < DISTANCE_LEVEL) {
                        foundNeutral = (obtainedflag & Flags.FOUND_NEUTRAL_EC) == Flags.FOUND_NEUTRAL_EC;
